...
def download_with_RFBP(src_path, dest_path, total_size, temp_size):
    '''
    断点续传(RFBP)
    ...
    r = requests.get(src_path, stream=True, verify=False, headers=headers) 
 ...
 ' ' * (50 - done), 100 * temp_size / total_size)) 
                sys.stdout.flush() 
    print() # 避免上面\r 回车符
...
def download_file(src_path, dest_path, src_size):
    ...
    if "Content-Length" in r.headers:
        ...
        print("src：{} {} {}".format(src_path,total_size, unit))
        print("dst: {} {} {}".format(dest_path, temp_size, unit))


    if (total_size > temp_size):
        download_with_RFBP(src_path, dest_path, total_size, temp_size)
...
def lookup_html(root_link, page_url):
    ...
    if r.status_code==200:
        ...
        for tr in soup.find_all('tr'):
            ...
            if a_list:
                src_path_list.append(root_link+'/'+a_list[0].get('href'))
                dest_path_list.append( "../xikao/{}-{}.zip".format(
index, td_list[0].string))
    ...
    for (src_path,dest_path, src_size) in zip(src_path_list, dest_path_list,
src_size_list):
        try:
